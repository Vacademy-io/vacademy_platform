...
public class InvoiceService {
    ...
    public List<InvoiceDTO> getInvoicesByUserId(String userId) {
        ...
        result.addAll(buildPendingSfpInvoiceDTOs(userId));
    ...
    private List<InvoiceDTO> buildPendingSfpInvoiceDTOs(String userId) {
        List<StudentFeePayment> sfps = studentFeePaymentRepository.findByUserIdAndStatusNot(userId, "PAID");
        List<InvoiceDTO> dtos = new ArrayList<>();
        for (StudentFeePayment sfp : sfps) {
            String status = sfp.getStatus();
            if (status == null || "PAID".equalsIgnoreCase(status) || "DELETED".equalsIgnoreCase(status)) continue;
            ...
            if (outstanding.signum() <= 0 && !"WAIVED".equalsIgnoreCase(status)) continue;
