...
class AssistantAgentService:
    ...
    def _build_system_prompt(
        ...
        return (
            ...
            "assessment schedule. For test schedules, use the help articles.\n"
            "- Overdue/outstanding fee figures are UNPAID balances — never present them as "
            "collections or revenue. If asked how much was collected/earned, say that data "
            "isn't available to you yet.\n"
            "- You CANNOT see assessment attempts or submissions. A missing score never means "
            "the student didn't attempt — say attempt status isn't visible to you.\n"
            ...
            "- Fees: `get_fee_dues` = what's outstanding/overdue; `get_payment_history` = what was "
            ...
            "EDITS (when enabled): calling update_learner_profile / manage_enrollment only "
            "PROPOSES — the user must press Confirm on the card. NEVER claim a change happened "
            "unless a later message in this conversation confirms it executed; typed 'yes' does "
            "not execute anything — point to the card. Restate exactly what will change before "
            "proposing.\n\n"
