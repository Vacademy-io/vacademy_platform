...
"""
    ...
    def get_video_branding(self, institute_id: str) -> Dict[str, Any]:
        ...
        """
        # Multi-tenant default — all three sections OFF.
        # The codebase serves multiple institutes and Vacademy branding
        # should NEVER leak into another tenant's video as a default. If
        # Vacademy itself wants the historical look, it can flip these
        # toggles on in its own VIDEO_BRANDING setting; the HTML payloads
        # are kept (off-by-default) for that case. Critically, the merge
        # at lines below preserves user-supplied `enabled` values, so a
        # user setting `enabled: True` with no html doesn't accidentally
        # inherit the Vacademy html from this default.
        DEFAULT_BRANDING = {
            "intro": {
                "enabled": False,
