...
def estimate_compile(
    ...
    transcribe_videos: bool, force: bool,
) -> Dict[str, Any]:
    ...
    for sid in dict.fromkeys(slide_ids):
        ...
        if src is None:
            rows.append({"slide_id": sid, "title": None, "kind": "other", "action": "unpublished", "compile": 0,
                         "transcription": 0, "minutes": 0, "images_max": 0, "total": 0, "note": "Not published"})
            continue
        ...
        row: Dict[str, Any] = {"slide_id": sid, "title": src.title, "kind": kind, "compile": 0.0, "transcription": 0.0,
                               "minutes": 0, "images_max": 0, "total": 0.0, "note": None, "text": None}
        if kind == "other":
        ...
        if kind == "video_upload":
            ...
            if cached:
                ...
                rows.append(row); continue
        elif kind in ("youtube", "ai_video", "pdf"):
            row["note"] = {"youtube": "Captions (free)", "ai_video": "Narration script (free)", "pdf": "PDF text (free)"}[kind]
        ...
        row["action"] = "compile"
        row["total"] = round(row["compile"] + row["transcription"], 2)
        rows.append(row)
    ...
    return {
        ...
        "prices": {"compile_slide": compile_price, "image": image_price, "transcription_per_minute": round(per_minute, 3),
                   "transcription_minimum": transcription_min},
        "balance": balance,
