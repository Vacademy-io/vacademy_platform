...
from app.mcp.constants import MCP_EXPOSED_TOOLS  # noqa: E402
...
# ── the exposed set ──────────────────────────────────────────────────────
def test_phase_one_exposes_only_the_institute_overview_tool():
    assert MCP_EXPOSED_TOOLS == ("get_institute_overview",)
...
def test_exposed_tools_are_all_read_only():
    assert all(spec.mode == "READ" for spec in adapter.exposed_specs())


def test_write_tools_are_not_exposed():
    write_tools = [name for name, spec in ASSISTANT_TOOLS.items() if spec.mode == "WRITE"]
    assert write_tools, "registry should contain write tools for this test to mean anything"
    assert not set(write_tools) & set(MCP_EXPOSED_TOOLS)
