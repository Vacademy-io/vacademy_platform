...
public class RestartAssessmentService {
    ...
    /**
     * Retrieves the new duration for an assessment attempt by validating and
     * updating student attempt data.
     *
     ...
     */
    public List<LearnerUpdateStatusResponse.DurationResponse> getNewDurationForAssessment(
            Optional<StudentAttempt> studentAttemptOptional,
            Assessment assessment,
            String requestAttemptJson) {

        if (studentAttemptOptional.isEmpty())
            throw new VacademyException("No Attempt Found");
        ...
        if (!Objects.isNull(requestAttemptJson)) {
            ...
            String attemptDataJson = updateStudentAttemptDataAndReturnLatest(requestAttemptJson,
                    studentAttempt.getAttemptData(), studentAttempt);
     ...
     */
    private String updateStudentAttemptDataAndReturnLatest(String requestAttemptJson, String savedAttemptJson,
            StudentAttempt studentAttempt) {
        if (Objects.isNull(savedAttemptJson) || isSavedDataOld(studentAttempt.getServerLastSync(),
                DateUtil.convertStringToUTCDate(attemptDataParserService.getClientLastSyncTime(requestAttemptJson)))) {
     ...
     * @return List of DurationResponse containing updated durations
     */
    private List<LearnerUpdateStatusResponse.DurationResponse> createDurationDistributionResponse(
            StudentAttempt studentAttempt,
    ...
    /**
     * Distributes the available time across assessment sections or questions based
     ...
     * @return List of DurationResponse containing updated durations
     */
    private List<LearnerUpdateStatusResponse.DurationResponse> distributeDuration(Assessment assessment, Long timeLeft,
            String attemptJson) {
        ...
        LearnerUpdateStatusResponse.DurationResponse assessmentDuration = LearnerUpdateStatusResponse.DurationResponse
                .builder()
                .id(assessment.getId())
        ...
        if (assessmentType.equals(DurationDistributionEnum.SECTION.name())) {
            ...
            if (!Objects.isNull(attemptJson)) {
                ...
                sections.forEach(sectionJson -> {
                    responses.addAll(createQuestionTimeDistribution(sectionJson, timeLeft,
                            attemptDataParserService.extractSectionIdFromSectionJson(sectionJson)));
                });
            } else {
                // No AttemptData available, fetch sections from the repository
                List<Section> allSections = sectionRepository.findByAssessmentIdAndStatusNotIn(assessment.getId(),
                        List.of("DELETED"));
    ...
    /**
     * Creates a time distribution for each question in a section based on the
     * remaining time.
     *
     * @param timeLeft  Remaining time available for the section.
     * @param sectionId ID of the section.
     * @return A collection of {@link LearnerUpdateStatusResponse.DurationResponse}
     *         objects representing time allocation for each question.
     */
    private Collection<? extends LearnerUpdateStatusResponse.DurationResponse> createQuestionTimeDistribution(
            String sectionJson, Long timeLeft, String sectionId) {

        // Retrieve questions from section attempt data if present, otherwise create an
        // empty list
        ...
        // If there are no attempted questions, handle the case where time needs to be
        // distributed among all questions
        ...
        Long totalAllocatedTime = questions.stream()
                .mapToLong(questionJson -> attemptDataParserService
                        .getQuestionDurationLeftInSecondsFromQuestionJson(questionJson) != null
                                ? attemptDataParserService
                                        .getQuestionDurationLeftInSecondsFromQuestionJson(questionJson)
                                : 0)
                .sum();
        ...
        return questions.stream().map(questionJson -> {
            long newTime = (totalAllocatedTime == 0)
                    ...
                    : (attemptDataParserService.getQuestionDurationLeftInSecondsFromQuestionJson(questionJson)
                            * timeLeft) / totalAllocatedTime; // Proportional distribution

            return new LearnerUpdateStatusResponse.DurationResponse(
                    attemptDataParserService.extractQuestionIdFromQuestionJson(questionJson),
                    DurationDistributionEnum.QUESTION.name(), newTime);
        }).collect(Collectors.toList());
    }

    /**
     * Handles the scenario where there are no attempted questions, distributing
     * time among all section questions.
     *
     * @param timeLeft  Remaining time available for the section.
     * @param sectionId ID of the section.
     * @return A collection of {@link LearnerUpdateStatusResponse.DurationResponse}
     ...
     * @return A collection of DurationResponse objects representing the time
    ...
    private Collection<? extends LearnerUpdateStatusResponse.DurationResponse> createSectionTimeDistribution(
            Long timeLeft, Assessment assessment, String attemptJson) {

        // Retrieve section attempt data if available, otherwise initialize an empty
        // list
        List<String> sections = (attemptJson != null && !attemptJson.isEmpty())
                ? attemptDataParserService.extractSectionJsonStrings(attemptJson)
                : new ArrayList<>();
        ...
        Long totalAllocatedTime = sections.stream()
                .mapToLong(sectionJson -> (sectionJson != null && !sectionJson.isEmpty())
                        ? attemptDataParserService.getSectionDurationLeftInSeconds(sectionJson)
                        : 0)
                .sum();
        ...
        return sections.stream().map(sectionJson -> {
            long newTimeInSeconds = (totalAllocatedTime == 0)
                    ...
                    : (((sectionJson != null && !sectionJson.isEmpty())
                            ? attemptDataParserService.getSectionDurationLeftInSeconds(sectionJson)
                            : 0) * timeLeft) / totalAllocatedTime;

            return new LearnerUpdateStatusResponse.DurationResponse(
                    attemptDataParserService.extractSectionIdFromSectionJson(sectionJson),
                    DurationDistributionEnum.SECTION.name(), newTimeInSeconds);
        }).collect(Collectors.toList());
    }
     ...
     *         allocated for each section.
     */
    private Collection<? extends LearnerUpdateStatusResponse.DurationResponse> handleCaseForNoSection(Long timeLeft,
            Assessment assessment) {
        ...
        return allSections.stream().map(section -> {
            long newTimeInSeconds = (totalAllocatedTimeInSeconds == 0)
                    ...
                    : ((section.getDuration() != null ? section.getDuration() : 0) * 60 * timeLeft)
                            / totalAllocatedTimeInSeconds;

            return new LearnerUpdateStatusResponse.DurationResponse(section.getId(),
                    DurationDistributionEnum.SECTION.name(), newTimeInSeconds);
        }).collect(Collectors.toList());
    }
