...
12. **LLM-centered elements shifted off-center in rendered MP4 even after animations completed** — shots that use the standard centering idiom (`position:absolute; top:50%; left:50%; transform:translate(-50%,-50%)`) and then animate the element with GSAP (`tl.fromTo(el, { y:30, scale:0.92 }, { y:0, scale:1 })`) lost the `-50%/-50%` centering in the rendered MP4. Symptom: container ends up anchored at its top-left at (50%, 50%) of the viewport instead of being centered, so content visibly drifts to the bottom-right of the frame even after the animation finishes. Root cause: GSAP seeds its internal `xPercent`/`yPercent` from `getComputedStyle(el).transform`, but the browser flattens CSS percent translates into a `matrix()` with pixel values — the original "percent" intent is lost. In the render-server's shadow DOM the timing of style application is tight enough that GSAP sometimes reads an empty/identity matrix and records `xPercent:0`/`yPercent:0`. Every subsequent write of transform then drops the centering shift. Fixed in [dispatcher_install_js.py](../../ai_service/app/ai-video-gen-main/dispatcher_install_js.py) by adding a one-time pre-scan in the scoped wrapper (just after `gsap = createScopedGsap(_shotTL)`, before the LLM's `originalCode` runs) that walks the shot's `<style>` rules, matches `transform: translate(X%, Y%)`, and calls `gsap.set(el, { xPercent:X, yPercent:Y, x:0, y:0 })` for each affected element. From that point on, GSAP knows the percent centering and preserves it through every fromTo/to/from. **Invariant: any time the shadow DOM dispatcher hosts an LLM-authored shot, percent translates in the shot's `<style>` MUST be pre-fed to GSAP via gsap.set so subsequent tweens preserve the centering.**
11. **Animations vanished after the dispatcher redeploy** — once #10 was fixed and the dispatcher actually installed, every shot's animations stopped running. Symptom in the render-worker logs: `total_gtl_children=0` for every shot (zero tweens registered with GSAP) plus repeated `[BROWSER EXCEPTION] Failed to execute 'replaceChild' on 'Node': Invalid regular expression: /circle(s*0(?:.0+)?(?:px|%|em|rem)?(?![d.])/: Unterminated group`. Root cause: the `scopedCode` template literal at [line 417](../../ai_service/app/ai-video-gen-main/dispatcher_install_js.py#L417) wraps the LLM-authored shot script and embeds several regex literals for collapsed-clip-path detection (added 2026-05-17 in `ce74fd8cd`). A **plain JS template literal silently drops unrecognized backslash escapes** (`\(` → `(`, `\s` → `s`, `\d` → `d`), so by the time `scopedCode` was assigned to `newScript.textContent`, every regex inside had been mangled into invalid syntax. The browser then refused to parse the script tag's text content (one `Unterminated group` SyntaxError per shot), the LLM's actual animation code never executed, and GSAP ended up with no tweens. Fixed by changing the template literal from `` const scopedCode = `...` `` to `` const scopedCode = String.raw`...` `` — `String.raw` preserves every backslash verbatim while still doing `${e.id}` interpolation. **Invariant: any template literal whose result is later parsed as JS source (or contains regex literals / escape sequences) MUST be a `String.raw` template, never a plain template literal.** Audit nearby for any other plain template literals that flow into `newScript.textContent`, `eval`, `Function(...)`, etc.
