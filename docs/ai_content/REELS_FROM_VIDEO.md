...
✅ **Caption-overlay dedup (Phase 2e from 2026-05-13 audit)** — when the director's text overlay (hook / micro_hook / loop_back / emphasis) is on screen, the karaoke caption block underneath produced a "two layers of similar text" visual mess (the audit screenshots showed yellow "TEAMWORK IS YOUR GREATEST STRENGTH" stacked on white "It's really always" karaoke). Fix: caption builder now accepts a `suppression_ranges: list[tuple[float, float]]` kwarg. A new `_collect_text_overlay_ranges()` helper extracts ranges from overlay shots whose `entry_meta.shot_type` is in `_CAPTION_SUPPRESSING_OVERLAY_TYPES` (text-only — broll / stat / motion_graphic are NOT included). `_block_overlaps_any()` checks open-interval overlap. Blocks that overlap any suppression range get dropped from the karaoke output. Visual overlays sit at a different z-band and don't suppress.

✅ **Stitched multi-clip bottom-half bgv for stacked layout (Phase 2c.9)** — biggest user-visible complaint from the 2026-05-13 audit: the bottom-half b-roll in `stacked_speaker_with_broll` was ONE Pexels clip on loop for the entire reel duration, no matter how the speaker's content shifted. Fix: LLM director now emits a `background_concepts: list[str]` SEQUENCE (2-5 Pexels queries) alongside the legacy single `background_concept` field. New `reels_bgv_stitch_service.py` (a) fetches each concept via the existing `find_b_roll` LRU-cached pipeline in parallel (`asyncio.gather`), (b) downloads each clip to a tempfile (`httpx` stream with 20s timeout per clip), (c) runs a single ffmpeg `filter_complex` that scales each input to 1080×960 (`scale + crop + setsar + fps=30 + setpts=PTS-STARTPTS`) and concats them, (d) uploads the stitched MP4 to S3. Per-segment duration = `reel_duration / N` clamped to `[3.0s, 10.0s]`. `generate_overlays` now returns a 3-tuple `(specs, single_concept, concept_sequence)`; `_build_storyline_overlays` propagates as a 4-tuple; `run()` tries the stitched path BEFORE the legacy single-concept fallback. Graceful degradation at every step: missing concepts validator returns None (caller falls through); <2 Pexels hits drops to single-concept path; ffmpeg failure → fallback; S3 upload failure → fallback. Env kill-switch `REELS_BGV_STITCH_DISABLED=1` disables without a redeploy. `extra_metadata.bgv_source="auto_pexels_stitched"` + `bgv_concepts` (the raw list) + `bgv_concept` (comma-joined) all land for audit. Real ffmpeg verification: 3 colored test clips (red/green/blue, 4s each) stitched with per_segment_s=3 produces a valid 9s 1080×960 MP4 with 1 video stream + no audio. Variable-length inputs (2s + 5s @ 3s cap) correctly produces 5s output (first clip uses its full 2s, second uses 3s). Total added cost: ~5-15s in DIRECTOR (parallel Pexels fetches dominate; ffmpeg concat is ~1-3s).
