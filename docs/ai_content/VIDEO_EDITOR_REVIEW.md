...
| ➕ B47 | B45's optimistic lock produced false conflicts + hard-blocked with a destructive reload | ✅ | Live-fire follow-up to B45. Two causes of *false* 409s: (1) the lock bumped `meta.revision` even on **unchecked** writes (no `expected_revision`) — so the pipeline view's `frame/update`, internal tooling, and old clients silently advanced the counter and tripped every open editor's next save; (2) `saveChanges` had no re-entrancy guard, so a double-clicked Save (or the auto-save-before-render racing a manual save) sent a stale `expected_revision` and 409'd against its own first bump. Fixes: `check_and_bump_revision` no longer bumps when `expected_revision is None` (two *editor* tabs both send it, so genuine concurrent-edit protection is retained; editor-vs-pipeline degrades to last-writer-wins, which is acceptable); `saveChanges` early-returns if already saving. UX change (user chose "warn but never lose work"): a 409 now sets a `saveConflict` flag and aborts the save **quietly** (unsaved edits kept in memory, undo history + localStorage buffer preserved) instead of throwing. New `SaveConflictNotice` strip under the toolbar offers **Save anyway** (`saveAnyway` refetches the live revision cache-busted, then retries → user's version overwrites) and **Discard & load latest** (`reloadFromServer` drops local edits + reloads). `handleSave` no longer false-toasts success on conflict; the render auto-save aborts instead of rendering with unsaved edits. `loadTimeline` now fetches `cache: 'no-store'` so a post-conflict reload can't read a stale CDN copy. |
