 ...
 *
 * Deliberately conservative: only `http(s)://`, `www.` and email/phone shapes are matched.
 * Bare domains are not, because ordinary prose ("Node.js", "etc.in", "3.5") would light up.
 *
 * Keep in sync with the learner app's `components/chat/linkify.ts`.
 */
...
/**
 * Peels punctuation that ended the *sentence* rather than the link — "…forms.gle/x4E6."
 * and "(see www.a.com)" should link `…/x4E6` and `www.a.com`, not swallow the period or
 * the closing paren. Balanced brackets inside a URL (a `(disambiguation)` path) are kept.
 */
