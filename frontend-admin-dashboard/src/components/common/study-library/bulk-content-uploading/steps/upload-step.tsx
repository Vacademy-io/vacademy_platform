...
export const UploadStep = ({ onZipSelected }: UploadStepProps) => {
    ...
    const slidesTerm = getTerminologyPlural(ContentTerms.Slide, SystemTerms.Slide);
    const slideTerm = getTerminology(ContentTerms.Slide, SystemTerms.Slide);
            ...
            >
                ...
                <div className="text-center">
                    ...
                    <p className="mt-1 text-caption text-neutral-500">
                        ...
                        {formatBytes(MAX_SINGLE_FILE_BYTES)} per file. PDF, Word, PowerPoint,
                        images, videos and link files are supported.
                    </p>
                </div>
                <div className="flex items-center gap-1 text-caption text-primary-500">
                    <UploadSimple className="size-4" />
                    <span>Folder names become your {courseTerm.toLowerCase()} structure</span>
                </div>
            </div>
                ...
                <div className="rounded-lg border border-neutral-200 bg-white p-4">
                    ...
                    <div className="mt-3 rounded-md border border-primary-100 bg-primary-50/50 p-3">
                        ...
                        <ul className="mt-2 flex flex-col gap-1">
                            <li className="flex items-center justify-between gap-2">
                                ...
                                <span className="text-caption text-primary-500">
                                    → 1st {slideTerm.toLowerCase()} “Introduction”
                                </span>
                            </li>
                            <li className="flex items-center justify-between gap-2">
                                <span className="font-mono text-caption text-neutral-600">
                                ...
                                <span className="text-caption text-primary-500">
                                    → 2nd {slideTerm.toLowerCase()} “Practice set”
                                </span>
                            </li>
                            <li className="flex items-center justify-between gap-2">
                                <span className="font-mono text-caption text-neutral-600">
                        ...
                        <p className="mt-2 text-caption text-neutral-500">
                            Works for folders too (01 {chapterTerm} A, 02 {chapterTerm} B…). The
                            number is removed from the name. In a {chapterTerm.toLowerCase()} that
                            already has {slidesTerm.toLowerCase()}, new ones are added after the
                            existing ones.
                        </p>
                    </div>
                    <p className="mt-2 text-caption text-neutral-500">
                        Put YouTube/external links in a links.txt file (one “Title | URL” per line).
                    </p>
                    {mode === 'multi' && (
                        <div className="mt-3 flex flex-col gap-2 rounded-md border border-neutral-100 bg-neutral-50 p-3">
                            <label className="flex items-center justify-between gap-4">
                                <span className="flex flex-col">
                                    ...
                                    <span className="text-caption text-neutral-500">
                                        Off = template includes all {templateCourseOptions.length}{' '}
                                        {getTerminologyPlural(
                                            ContentTerms.Course,
                                            SystemTerms.Course
                                        ).toLowerCase()}
                            ...
                            {pickTemplateCourses && (
                                <MultiSelect
                                    ...
                                    placeholder={`Search & select ${getTerminologyPlural(ContentTerms.Course, SystemTerms.Course).toLowerCase()}…`}
                                />
                            )}
                        </div>
                    )}
                    ...
                    <p className="mt-1 text-caption text-neutral-400">
                        Folders named after your existing structure — just drop files in and upload
                        it back.
                    </p>
                </div>
                ...
                <div className="flex flex-col gap-4 rounded-lg border border-neutral-200 bg-white p-4">
                    ...
                    <label className="flex items-center justify-between gap-4">
                        <span className="flex flex-col">
                            ...
                            <span className="text-caption text-neutral-500">
                                Off = {slidesTerm.toLowerCase()} are saved as drafts for review
                            </span>
                        </span>
                        <Switch
                    ...
                    <label className="flex items-center justify-between gap-4">
                        <span className="flex flex-col">
                            ...
                            <span className="text-caption text-neutral-500">
                                Don’t re-create {slidesTerm.toLowerCase()} whose title already
                                exists in the {chapterTerm.toLowerCase()}
                            </span>
                        </span>
                        <Switch
                            checked={options.skipDuplicateTitles}
                            onCheckedChange={(checked) =>
                                setOptions({ skipDuplicateTitles: checked })
                            }
                        />
                    </label>
                </div>
