...
class TemplateCacheService {
    ...
    async getTemplates(type?: MessageTemplate['type']): Promise<MessageTemplate[]> {
    ...
    private async loadTemplates(type?: MessageTemplate['type']): Promise<MessageTemplate[]> {
        try {
    ...
    clearCache(type?: MessageTemplate['type']): void {
        if (type) {
            const cacheKey = type;
    ...
    getCachedTemplates(type?: MessageTemplate['type']): MessageTemplate[] | null {
        const cacheKey = type || 'all';
        return this.cache.get(cacheKey) || null;
    }

    // Check if cache is fresh
    isCacheFresh(type?: MessageTemplate['type']): boolean {
        const cacheKey = type || 'all';
        const now = Date.now();
        const lastFetchTime = this.lastFetch.get(cacheKey) || 0;
