 ...
 * (see ai_service render_worker /pptx-anim-jobs). Each original slide is a group
 * of ordered step images; advancing WITHIN a slide cross-fades (so an entrance
...
export default function DeckPlayer({ baseUrl }: DeckPlayerProps) {
  ...
  // Preload neighbouring step images so the cross-fade is instant — without this
  // the next snapshot downloads on click and flashes empty mid-fade.
  useEffect(() => {
    if (!steps) return;
