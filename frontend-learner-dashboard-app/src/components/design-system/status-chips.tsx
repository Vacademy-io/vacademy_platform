...
const statusIcons: Record<StatusType, React.ReactNode> = {
    SUCCESS: <Check className="text-white bg-success-600 rounded-full sm:size-4 size-3 sm:p-1 p-px" />,
    DANGER: <ExclamationMark className="text-white bg-danger-600 rounded-full sm:size-4 size-3 sm:p-1 p-px" />,
    WARNING: <ExclamationMark className="text-white bg-warning-600 rounded-full sm:size-4 size-3 sm:p-1 p-px" />,
    INFO: <X className="text-white bg-neutral-500 rounded-full sm:size-4 size-3 sm:p-1 p-px" />,
}
export const StatusChip = ({text, textSize, status, showIcon=true}: {text: string, textSize: string, status: StatusType, showIcon?: boolean}) => {
    return(
        <div className={`border w-fit ${statusColors[status]} rounded-md sm:px-2 sm:py-1 ${textSize} flex gap-1 items-center sm:text-regular text-2xs p-1`}>
