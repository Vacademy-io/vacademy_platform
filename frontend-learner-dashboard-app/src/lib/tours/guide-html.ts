 ...
 *
 * The chapters included follow the tutorial checkpoints the institute has
 * enabled in Student Display settings; the admin dashboard keeps an identical
 * copy of this builder so both portals produce the same document. The output
 * is strict XHTML rendered server-side by openhtmltopdf
 * (admin-core-service /institute/v1/tutorial-guide/render-pdf), so keep markup
 * well-formed (self-closed tags) and styling inline in the <style> block —
 * no external resources except the logo <img>, which the backend inlines.
 */
...
export interface TutorialGuideBranding {
  ...
  logoUrl?: string | null;
  // Any CSS color; defaults to the Vacademy primary orange
...
function buildChapters(): GuideChapter[] {
  ...
  const chapter = getTerminology(ContentTerms.Chapters, SystemTerms.Chapters);
  const chapters = getTerminologyPlural(
  ...
  return [
    {
      ...
      intro:
        'Everything in the app is reachable from two places: your dashboard (the home screen) and the main navigation menu.',
      steps: [
    ...
    },
    {
      ...
      intro:
        'Assessments are timed tests assigned by your institute. Everything from starting an attempt to reviewing results happens in the app.',
      steps: [
    ...
    },
    {
      ...
      intro:
        'The app keeps a running record of everything you study and every test you take, so you always know where you stand.',
      steps: [
        ...
        'Certificates (when your institute enables them) unlock automatically once you cross the completion threshold — look for the certificate option inside the completed ' +
          `${lc(course)}.`,
      ],
      tips: [
...
export function buildTutorialGuideHtml(
  ...
  const chapters = buildChapters().filter((c) =>
    enabledSections.includes(c.key)
  );
