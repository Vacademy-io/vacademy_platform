...
import { Crown, Medal, Trophy, ShieldCheck } from "@phosphor-icons/react";
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-end gap-2">
      {first ? (
        ...
        <Medal weight="fill" className={cn("size-5", place === 2 ? "text-neutral-400" : "text-warning-700")} />
      )}
      ...
      <div className="w-full px-1 text-center">
        <p className="truncate text-caption font-bold text-foreground">{entry.name}</p>
        <p className="text-caption font-semibold tabular-nums text-primary-500">{entry.points} pts</p>
        {entry.badgeCount > 0 && (
          <p className="mt-0.5 inline-flex items-center gap-0.5 text-caption text-warning-600">
...
function ListRow({ entry }: { entry: LeaderboardEntry }) {
  return (
    <div className="flex items-center gap-3 rounded-xl border border-neutral-100 bg-card px-3 py-2.5 transition-colors hover:border-primary-100">
      ...
      {entry.badgeCount > 0 && (
        <span className="inline-flex items-center gap-1 rounded-full bg-warning-50 px-2 py-0.5 text-caption font-semibold text-warning-600">
...
function PublicLeaderboardPage() {
  ...
  const busy = brandingLoading || loading;
