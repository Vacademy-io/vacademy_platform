 ...
 * learner got an empty iframe that never played.
 *
...
export const extractVimeoId = (url: string): string =>
  url.match(VIMEO_URL_PATTERN)?.[1] || "";

export const isVimeoUrl = (url: string): boolean =>
  VIMEO_URL_PATTERN.test(url);

export const resolveVideoSourceType = (
  videoSlide: VideoSlideLike
