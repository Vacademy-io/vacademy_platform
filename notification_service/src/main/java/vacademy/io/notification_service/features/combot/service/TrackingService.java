...
public class TrackingService {
    ...
    private final EngagementTriggerService engagementTriggerService;
    private final ObjectMapper objectMapper;

    @Transactional
    public void logTrackingEvent(TrackingRequest request) {
        try {
            // 1. Save the tracking log entry
            NotificationLog logEntry = createLogEntry(request);
            notificationLogRepository.save(logEntry);
            
            log.debug("Logged tracking event: type={}, user={}, source={}", 
                    request.getType(), request.getUserId(), request.getSourceId());

            // 2. Check and trigger engagement-based messages (inline)
            engagementTriggerService.checkAndTriggerEngagement(request);
    ...
    /**
     * Create notification log entry from tracking request
     */
    private NotificationLog createLogEntry(TrackingRequest request) {
