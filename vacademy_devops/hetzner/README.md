...
| `../vacademy-services/values-prod-hetzner.yaml` | Hetzner prod chart values (delta vs current Linode prod) | ⏳ |
...
3. **Snapshot of current Linode prod values** (or `scripts/export-live-config.sh` output) so we know which secrets to carry over.
